...
class YamlLoader(object):
    ...
    def create_formula_process(self, definition, yaml_structure, sim_control, constants, prototypes):
        ...
        if 'id' in definition:
            pid = definition['id']
            # check unique - else throw exception
        else:
            pass
            # assing definition
            # write the updated yaml back to file, with logging
    ...
    def parse_yaml_structure(self, yaml_structure, sim_control) -> Tuple[Dict[str, FormulaProcess], Dict[str, str]]:
        ...
        for defintion in yaml_process_structs:
            ...
            if 'type' in defintion and defintion['type'] == 'prototype':
                prototypes[defintion['name']] = defintion
                continue

            processes[defintion['name']] = self.create_formula_process(defintion, yaml_structure, sim_control,
                                                                       constants, prototypes)
            link_map[defintion['name']] = defintion.get('link_to', [])
